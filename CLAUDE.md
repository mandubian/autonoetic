...
- `docs/internals/storage/layers-and-artifacts.md` — What a layer actually is: a content-addressed **directory image bound to a mount path**, not a package set. Why that makes conda/compiled output first-class, why `resolved_packages` is scanned provenance (legitimately empty for a compiled layer) rather than a manifest, the composition table (Python composes on `sys.path`; Node ESM ignores `NODE_PATH` and composes only via the directory walk; Cargo/Go resolve at build time), the rule that a layer is **the output of one resolution, never an input to another**, reuse being by reference with no discovery mechanism, and the unimplemented hermetic-capsule layer embedding
