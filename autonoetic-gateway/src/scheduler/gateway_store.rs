...
use autonoetic_types::agent_revision::{
    AgentAliasRecord, AgentRevisionRecord, AgentRevisionStatus, PromotionRecord,
    SessionAgentBinding,
};
    ...
    pub fn list_promotion_history(&self, agent_id: &str) -> Result<Vec<PromotionRecord>> {
        ...
        let rows = stmt.query_map(params![agent_id], |row| {
            ...
            let kind = match kind_str.as_str() {
                ...
                "Rollback" => autonoetic_types::agent_revision::PromotionKind::Rollback,
                _ => autonoetic_types::agent_revision::PromotionKind::Promote,
            };
            Ok(PromotionRecord {
...
pub fn bootstrap_agents(
    ...
    for entry in std::fs::read_dir(&config.agents_dir)? {
        ...
        let mut file_map: std::collections::BTreeMap<String, Vec<u8>> =
        ...
        let _short_id = store.insert_agent_revision_transactional(&rev)?;

        let promotion_id =
            mint_hashed_prefixed_id("prom-", &format!("{}-{}-{}", agent_id, revision_id, now));

        let _prev = store.atomic_promote(
            &agent_id,
            &revision_id,
            &promotion_id,
            "bootstrap",
