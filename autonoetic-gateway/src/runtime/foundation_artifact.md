...
- When receiving a task that references a file or artifact, use `content_read(name_or_handle)` to retrieve it.
...
- Evaluator/auditor uses `artifact_inspect(artifact_id)` to review the exact files.
