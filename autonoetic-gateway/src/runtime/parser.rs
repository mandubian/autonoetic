...
use autonoetic_types::agent::{
    AdapterProvenance, AgentEgressManifest, AgentIO, AgentIdentity, AgentManifest, AgentSkillsImportMetadata, CompressionConfig,
...
"#;
    ...
    #[test]
    fn test_parse_adapter_provenance_metadata_shape() {
        let content = r#"---
...
metadata:
  autonoetic:
    ...
    adapter:
      ...
      base_revision_digest: "rev_sha256:abc123"
      generated_at: "2026-08-27T10:30:00Z"
      schema_notes: ["accepts: compatible", "returns: mapping required"]
      generator: "agent-adapter.default"
---
# Wrapper
"#;
        let (manifest, _body) = SkillParser::parse(content).expect("should parse");
        let adapter = manifest.adapter.expect("adapter provenance should parse");
        assert_eq!(adapter.base_agent_id, "researcher.default");
        assert_eq!(
            adapter.base_revision_digest.as_deref(),
            Some("rev_sha256:abc123")
        );
        assert_eq!(adapter.generated_at.as_deref(), Some("2026-08-27T10:30:00Z"));
        assert_eq!(adapter.schema_notes.len(), 2);
        assert_eq!(adapter.generator.as_deref(), Some("agent-adapter.default"));
    }

    #[test]
    fn test_parse_adapter_provenance_minimal_and_bare_root_shape() {
        // The shape `generate_wrapper.py` emits today: only the three original
        // fields, and — in the bare-root form — no `metadata` wrapper at all.
        // Both must parse; absent optional fields stay `None`/empty.
        let content = r#"---
version: "1.0"
runtime:
  engine: "autonoetic"
...
adapter:
  base_agent_id: "researcher.default"
  generated_at: "2026-08-27T10:30:00Z"
  schema_notes: ["accepts: compatible"]
---
# Wrapper
"#;
        let (manifest, _body) = SkillParser::parse(content).expect("should parse");
        let adapter = manifest.adapter.expect("adapter provenance should parse");
        assert_eq!(adapter.base_agent_id, "researcher.default");
        assert_eq!(adapter.base_revision_digest, None);
        assert!(adapter.generator.is_none());
        assert_eq!(adapter.schema_notes, vec!["accepts: compatible".to_string()]);
    }
