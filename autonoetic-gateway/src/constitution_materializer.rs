...
//!    unsigned lock, a `provenance.json` linking every applied proposal
//!    back to its ID and adjudication, and a **scaffold `RATIFY.md`** — the
//!    mechanical facts filled in, the justification sections left as
//!    explicit `RATIFY-PLACEHOLDER` markers the operator completes.
//!
...
//! Status, Relation on added rows, every RATIFY justification), sign via
//! `recompute_lock.py`, and activate through the ordinary ceremony. A
//! materialized draft that was never signed is indistinguishable from a
...
/// Every `RATIFY-PLACEHOLDER` in the scaffold shares this visible marker so a
/// reviewer skimming rendered markdown sees what is still owed, plus a
/// greppable HTML comment a future guard could fail on.
const RATIFY_PLACEHOLDER_BANNER: &str = "\
> ⚠ **OPERATOR — RATIFICATION JUSTIFICATION OWED HERE.** This scaffold was
> generated mechanically; the text below it must be written by the operator
> before signing.";

/// Build the scaffold `RATIFY.md` for a candidate version.
///
/// The mechanical facts (which proposals, their filing justifications, their
/// adjudication, the digests, the ceremony commands) are filled in; the
/// ratification *arguments* are left as explicit `RATIFY-PLACEHOLDER`
/// markers, because the justification for a law change is the operator's
/// voice, not the gateway's. A batch that touches an entrenched clause
/// (the correction core — `enforcement_register::entrenched_clauses()`)
/// gets a mandatory justification section per the amendment process.
fn ratify_scaffold(
    candidate_version: &str,
    base_version: &str,
    base_digest: &str,
    ...
    out.push_str(&format!(
        "# RATIFY.md — Constitution Version {candidate_version}\n\n\
         <!-- Generated by the amendment materializer (#810) from {base_version}. This file \
         is a SCAFFOLD, not a ratification record: every `RATIFY-PLACEHOLDER` marker below \
         must be completed by the operator before signing. The machine-linkable record of \
         what changed and how it was adjudicated lives in provenance.json alongside this \
         file. -->\n\n"
    ));

    out.push_str(&format!(
        "## Summary\n\n\
         Mechanically materialized from **{base_version}** — {applied} proposal(s) applied. \
         Base digest `{base_digest}` → candidate digest `{candidate_digest}`.\n\n\
         {RATIFY_PLACEHOLDER_BANNER}\n\n\
         <!-- RATIFY-PLACEHOLDER: summary -->\n\n\
         State in one or two sentences what this amendment is *for*. The mechanical facts \
         are above and in provenance.json; this summary is the ratification argument, and it \
         is yours.\n\n",
        applied = proposals.len(),
    ));

    out.push_str("## Applied proposals\n\n");
    out.push_str("| Proposal | Kind | Target | Proposer | Decided by | Decided at |\n");
    out.push_str("|---|---|---|---|---|---|\n");
    for p in proposals {
        out.push_str(&format!(
            "| {} | {} | {} | {} | {} | {} |\n",
            ...
            p.proposer_agent_id,
            p.decided_by.as_deref().unwrap_or("—"),
            p.decided_at.as_deref().unwrap_or("—"),
        ));
    }
    out.push_str("\n");
    for p in proposals {
        let justification = one_line(p.justification.trim());
        out.push_str(&format!(
            "- **{}** — filed justification:\n  > {}\n",
            p.proposal_id, justification
        ));
    }
    out.push_str(&format!(
        "\n{RATIFY_PLACEHOLDER_BANNER}\n\n\
         <!-- RATIFY-PLACEHOLDER: proposal-ratifications -->\n\n\
         For each proposal above, record why the *decider* ratified it — not just why it \
         was filed. A filed reason is an agent's argument; a ratification reason is the \
         operator's judgment that the argument is law-worthy.\n\n"
    ));

    // Entrenched clauses: the amendment process requires an explicit, dated
    // justification recorded in THIS file when a correction-core clause is
    // weakened or removed. The materializer knows mechanically which clauses
    // a batch touches, so it raises the section itself — what it cannot do
    // is write the justification.
    let entrenched: Vec<&str> = crate::enforcement_register::entrenched_clauses();
    let touched: Vec<&str> = edits
        .iter()
        .filter_map(|e| {
            let target = e.target_id.as_deref()?;
            // A modify of an entrenched clause must at least be *checked*
            // against its protective force; a remove is the weaken case the
            // amendment process names outright.
            entrenched.contains(&target).then_some(target)
        })
        .collect();
    if !touched.is_empty() {
        let list = touched.join(", ");
        out.push_str(&format!(
            "## Entrenched clauses touched\n\n\
             This amendment modifies entrenched correction-core clause(s): **{list}**. Per \
             the amendment process, an amendment that weakens or removes one requires an \
             explicit, dated justification recorded HERE, on top of the ordinary sign-off \
             requirements. If the amendment only *strengthens* the clause, say so plainly — \
             that is the one answer that ends this section.\n\n\
             {RATIFY_PLACEHOLDER_BANNER}\n\n\
             <!-- RATIFY-PLACEHOLDER: entrenched-justification -->\n\n"
        ));
    }

    // DRAFT rows: added clauses carry TBD Source/Status/Relation cells until
    // the operator classifies them. Surface that obligation where the
    // classification will be defended.
    let added: Vec<&ProposalEdit> = edits
        .iter()
    ...
    if !added.is_empty() {
        let list = added
            ...
            .join(", ");
        out.push_str(&format!(
            "## Draft rows completed by the operator\n\n\
             The following clauses were inserted as DRAFT rows with TBD Source, Status, and \
             Relation cells: **{list}**. Completing that classification in \
             `constitution.md` is part of this ratification — the gateway declined to \
             author it.\n\n\
             {RATIFY_PLACEHOLDER_BANNER}\n\n\
             <!-- RATIFY-PLACEHOLDER: draft-classification -->\n\n"
        ));
    }

    out.push_str(&format!(
        "## Signing and activation\n\n\
         1. **Sign** — `python3 docs/constitution/recompute_lock.py --version {candidate_version} \\\n\
            \x20  --signing-sk-b64 \"$AUTONOETIC_CONSTITUTION_SIGNING_SK_B64\"`, then update \
            `docs/constitution/CURRENT`.\n\
         2. **Activate** — flip `ACTIVE_CONSTITUTION_VERSION` in `autonoetic-types/src/config.rs` \
            and the runtime snapshot paths in `docs/reference/config.md`; repoint the \
            active-version pin test.\n\
         3. **Re-bless** — the lock, glossary, register-doc, and law-table bless tests; then \
            the full `tests/constitution` domain binary.\n\n\
         The candidate lock is unsigned; nothing is law until step 1 succeeds and the \
         ceremony completes.\n"
    ));
    out
}

/// Collapse a justification to one line for safe embedding in markdown prose.
...
";
    ...
    fn materialize_writes_candidate_dir_with_unsigned_lock_and_provenance() {
        ...
        // Scaffold RATIFY.md: mechanical facts present, justifications owed.
        // cprop-a modifies P-8.1, which IS a correction-core clause — the
        // entrenched-justification section must be raised. cprop-b adds a
        // row — the draft-classification section must be raised too.
        let ratify = std::fs::read_to_string(report.candidate_dir.join("RATIFY.md")).unwrap();
        assert!(ratify.starts_with("# RATIFY.md — Constitution Version 2026.01.02"));
        assert!(ratify.contains("Mechanically materialized from **2026.01.01**"));
        assert!(ratify.contains("| cprop-a | modify_rule | P-8.1 |"));
        assert!(ratify.contains("RATIFY-PLACEHOLDER: summary"));
        assert!(ratify.contains("RATIFY-PLACEHOLDER: proposal-ratifications"));
        assert!(ratify.contains("RATIFY-PLACEHOLDER: entrenched-justification"));
        assert!(ratify.contains("RATIFY-PLACEHOLDER: draft-classification"));
        assert!(ratify.contains("recompute_lock.py --version 2026.01.02"));
    }

    #[test]
    fn scaffold_omits_conditional_sections_when_not_applicable() {
        let temp = tempfile::tempdir().unwrap();
        let versions = temp.path().join("versions");
        let base_dir = versions.join("2026.01.01");
        std::fs::create_dir_all(&base_dir).unwrap();
        ...
        // P-8.2 is not entrenched; no add_* in the batch.
        let report = materialize_candidate_version(
            &versions,
            "2026.01.01",
            "2026.01.03",
            &[proposal("cprop-m", "modify_rule", Some("P-8.2"), Some("Amended."))],
        )
        .unwrap();
        let ratify = std::fs::read_to_string(report.candidate_dir.join("RATIFY.md")).unwrap();
        assert!(!ratify.contains("RATIFY-PLACEHOLDER: entrenched-justification"));
        assert!(!ratify.contains("RATIFY-PLACEHOLDER: draft-classification"));
        assert!(ratify.contains("RATIFY-PLACEHOLDER: summary"));
        assert!(ratify.contains("cprop-m"));
    }
