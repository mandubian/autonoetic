...
//! - opt-in via `response_contract.repair.auto: true` enables bounded repair
...
async fn repair_disabled_by_default_without_manifest_opt_in() -> anyhow::Result<()> {
    ...
    install_validation_agent(&workspace.agents_dir, "repair.default_off.agent")?;
    ...
    assert_eq!(
        *call_count.lock().unwrap(),
        1,
        "without response_contract.repair.auto opt-in, gateway must not run repair turns"
    );
    Ok(())
}
...
async fn repair_opt_in_runs_bounded_repair_turn() -> anyhow::Result<()> {
    ...
    install_validation_agent(&workspace.agents_dir, "repair.opt_in.agent")?;
...
async fn repair_attempts_are_capped_by_system_ceiling() -> anyhow::Result<()> {
    ...
    install_validation_agent(&workspace.agents_dir, "repair.ceiling.agent")?;
    ...
    let metadata = serde_json::json!({
        "response_contract": {
            "required_artifacts": ["required.md"],
            "repair": {"auto": true, "max_attempts": 5},
            "validation_max_duration_ms": 5000
        }
    });
