...
#[tokio::test]
async fn test_tier2_memory_cross_agent_sharing() {
    ...
    let mem_reader =
        Tier2Memory::open_sqlite(&gateway_dir, "reader-agent")
            .unwrap();
...
async fn test_tier2_memory_unauthorized_access_denied() {
    ...
    let mem_a =
        Tier2Memory::open_sqlite(&gateway_dir, "agent-a").unwrap();
    ...
    let mem_b =
        Tier2Memory::open_sqlite(&gateway_dir, "agent-b").unwrap();
...
async fn test_tier2_memory_search_with_visibility() {
    ...
    let mem = Tier2Memory::open_sqlite(&gateway_dir, "search-agent")
        .unwrap();
...
async fn test_tier2_memory_global_visibility() {
    ...
    let mem_owner =
        Tier2Memory::open_sqlite(&gateway_dir, "owner-agent").unwrap();

    let mem_reader1 =
        Tier2Memory::open_sqlite(&gateway_dir, "reader-agent-1")
            .unwrap();

    let mem_reader2 =
        Tier2Memory::open_sqlite(&gateway_dir, "reader-agent-2")
            .unwrap();
...
async fn test_tier2_memory_only_owner_can_make_global() {
    ...
    let mem_non_owner =
        Tier2Memory::open_sqlite(&gateway_dir, "non-owner-agent")
            .unwrap();
...
async fn test_tier2_memory_list_scopes() {
    ...
    let mem =
        Tier2Memory::open_sqlite(&gateway_dir, "scope-agent").unwrap();
...
#[tokio::test]
async fn test_tier2_memory_search_by_tags_cross_agent_shared() {
    ...
    let writer = Tier2Memory::new(
        Arc::new(SqliteMemoryStore::new(Arc::clone(&store))),
        "writer-agent",
    );
    ...
    let reader = Tier2Memory::new(
        Arc::new(SqliteMemoryStore::new(store)),
        "reader-agent",
    );
    let found = reader
        .search_by_tags(
