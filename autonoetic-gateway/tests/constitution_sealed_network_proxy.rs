...
async fn sealed_proxy_returns_fixture_on_hit() {
    ...
    let handle = start_sealed_proxy(SandboxNetworkPolicy::Sealed, loader)
        .await
...
async fn sealed_proxy_rejects_normal_policy_at_start() {
    ...
    let dir = tempdir().unwrap();
    let loader = Arc::new(FixtureLoader::new(dir.path()));
    let err = start_sealed_proxy(SandboxNetworkPolicy::Normal, loader)
        .await
...
async fn sealed_proxy_shuts_down_cleanly() {
    ...
    let h1 = start_sealed_proxy(SandboxNetworkPolicy::Sealed, loader1)
        .await
    ...
    let h2 = start_sealed_proxy(SandboxNetworkPolicy::Sealed, loader2)
        .await
