...
`agent_revision_create({agent_id, artifact_id: <post-packager artifact_ref>})` and pass
...
`agent_revision_create({agent_id, artifact_id: <post-packager artifact_ref>})`. It
returns `{revision_id: "rev_sha256:...", short_ref: "<agent>@rev_<short>", status: "created"}`
(or reuses an existing candidate revision with the same content digest). Pass that
`revision_id` to `federation.escalate` — for **both new and existing agents**. This
avoids the unseeded-escalation path, which reads capabilities from the artifact's
`SKILL.md` at escalate time and fails opaquely if the frontmatter is missing or invalid.
Do **not** omit `revision_id` and do **not** invent a placeholder.
