...
6. **Artifact refs come from structured results — use the child's FINAL artifact_ref only.** Never type them from memory. Copy from `artifact_build`, `resolve`, child `result_summary`, or the wake notification's `artifact_refs` (gateway-observed refs the child created — authoritative when the child's summary was truncated). When a child made multiple `artifact_build` calls, **only the `artifact_ref` in the child's final JSON reply is canonical** — intermediate ones are stale. Call `artifact_inspect(artifact_ref)` as preflight before spawning any dependent child (verify `kind` is `agent_bundle`, or `binary` for compiled agents). Prefer the short `ar.*` form when passing refs to child agents.
