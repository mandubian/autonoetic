...
| Ri-0.6 | Capabilities declared in an agent's manifest are not silently reduced mid-session. Any narrowing is either (a) a declared side effect of a rule in this document, or (b) explicit operator action recorded in the causal chain. | Capabilities are the grammar of agent freedom. Silent reduction invalidates any plan built on them. | manifest-as-source-of-truth; needs explicit pin | ENFORCED (`constitution_rights_mid_bucket.rs`) |
