...
| Ri-0.3 | Every rejection names the rule ID that caused it. No agent is ever told "denied" without being told why. | Rejection without explanation is indistinguishable from arbitrary authority. The constitutional test: if you were denied, you can look up which rule you ran into. | `causal_events.enforced_rules` persisted on every decision event (`session_tracer`, gateway-store migration v20) | ENFORCED |
