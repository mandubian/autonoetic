...
| Ri-0.3 | Every rejection names the rule ID that caused it. No agent is ever told "denied" without being told why. | Rejection without explanation is indistinguishable from arbitrary authority. The constitutional test: if you were denied, you can look up which rule you ran into. | `causal_events.enforced_rules` persisted on every decision event (`session_tracer`, gateway-store migration v21); key denial paths carry rule IDs; remaining paths (scheduler, evaluation, write-path) still use untagged errors — see I-6 for the cross-cutting invariant which is ENFORCED | PARTIAL (core mechanism ENFORCED via R+++3; remaining gaps in scheduler/evaluation/write-path denial paths) |
