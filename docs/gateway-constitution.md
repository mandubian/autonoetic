...
| R-8.15 | Causal-chain append is `fsync`-durable before any state transition that depends on it. | (R+6) | `causal_chain.rs:83` `gateway_store/mod.rs:112` | ENFORCED |
...
| R++5 | Every tool call carries an `intent` field (free-text, 1-2 sentences) describing the agent's reason for invoking the tool. Persisted to the causal chain alongside args. Missing intent on privileged tool classes is an error. | P0 | §8 Audit |
