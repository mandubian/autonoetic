...
needed. Classifying all 221 clauses found **180 of the 182 `P-*` bind the
enforcer**. The exceptions are two: `P-2.9` binds the reasoner, and `P-2.21`
binds the decider — an agent-decider that cannot decide owes escalation, which
is the one operational rule that binds a *seat* rather than a party. A
...
| P-9.13 | Agent bundle signatures are verified at `agent_revision_create`. | spec-install-pipeline-hardening.md | `agent_revision.rs`, `autonoetic-gateway/tests/constitution/install_signature.rs` | ENFORCED | enforcer · none · preventive |
