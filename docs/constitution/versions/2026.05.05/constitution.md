...
| ID | Rule | Source | Enforcement | Status |
|---|---|---|---|---|
| R-2.1 | Remote network access across all networked tools (`sandbox_exec`, `credential.*`, `web.*`) is statically detected and blocks pending approval rather than hard-denying. | remote-access-approval.md | `runtime/tools/sandbox.rs`, `credential.rs`, `web.rs` | ENFORCED |
