...
- [Session budgets](../reference/session-budget.md) — YAML limits
- [Budget management](../reference/budget-management.md) — enforcement flow, OpenRouter catalog timing
...
Session-scoped LLM/tool/token/wall-clock/optional USD caps are configured in YAML as `session_budget` (see [session-budget.md](../reference/session-budget.md)).
