...
- [autonoetic-gateway](#autonoetic-gateway) — crate-level; per-file detail in [`gateway.md`](gateway.md)
...
Related: [session budgets](../reference/budgets.md) — YAML limits, enforcement flow, OpenRouter catalog timing.
...
The core runtime: transport, execution engine, policy, storage, scheduler.

| Layer | Path | Owns |
|---|---|---|
| Server | `src/server/` | OFP + JSON-RPC + HTTP listeners, message routing, peer registry |
| Runtime | `src/runtime/` | The agent execution loop, tool registry and dispatch, prompt composition, context governor |
| Router | `src/router.rs` | JSON-RPC method dispatch to the execution service |
| Execution | `src/execution.rs` | `GatewayExecutionService` — spawn, emergency stop, degraded mode, preset resolution |
| Scheduler | `src/scheduler/` | Background tasks, cron bridge, and the `GatewayStore` (SQLite) |
| Causal chain | `src/causal_chain/` | Hash-chained audit trail |
| LLM | `src/llm/` | Provider drivers and the egress chokepoint |
| Policy | `src/policy.rs` | Capability validation ahead of any privileged operation |

**Per-file detail with call-level annotations lives in
[`gateway.md`](gateway.md)** — that is the single gateway module map. This table
is deliberately crate-level: two per-file listings of the same code in two
formats is what drifted before.

HTTP endpoints are specified in [`../reference/http-api.md`](../reference/http-api.md).
...
Session-scoped LLM/tool/token/wall-clock/optional USD caps are configured in YAML as `session_budget` (see [budgets.md](../reference/budgets.md)).
