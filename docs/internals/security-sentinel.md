...
Configuration knobs are documented in [`docs/reference/config.md`](../reference/config.md) under "Security Sentinel".
...
- The **separation-of-powers model** (see `docs/concepts/separation-of-powers.md`) applies one level up: the gateway polices agents, the sentinel polices the system-as-a-whole. The primitives are the same — propose, don't enforce.
