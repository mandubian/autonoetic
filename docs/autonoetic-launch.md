...
For the deep version, see [`autonoetic-concepts-for-beginners.md`](autonoetic-concepts-for-beginners.md).
...
| One agent, one model | Many agents, model-agnostic |
...
5. **Open the trace.** `autonoetic trace list`. Who did what, with what authority.
...
cargo run -p autonoetic -- trace list  # the receipts
```
...
- "Human and agent edit through the same machinery." (true, shipped)
...
| Security communities | The credential-isolation story, the sandbox model |
| Agent / LLM-tooling circles | Multi-agent durability, capsules, model-agnostic presets |
| Long-form (blog / talk) | "Actors as citizens" — the constitutional thesis |

Match the use-case to the room. Security wants story 2 and 3. Builders want 1
and 4. Visionaries want 5 and 6.
