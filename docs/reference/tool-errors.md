...
## Delegation failure classes

Three `FailureClass` values (carried on `ToolError.failure_class`) exist for
failures of *delegated* work, where the parent needs to know that retrying the
identical call cannot help:

| `failure_class` | Means | Retry |
|---|---|---|
...
`retry_advice` carries the policy separately from the class — `wait`,
`retry_same_stage`, `retry_after_external_recovery`, `do_not_retry`,
`escalate_human`, `fix_artifact_then_retry`. Keeping *what happened* apart from
*what may be done about it* is what lets a parent branch on one field, and what
stops "recoverable" from being read as "retry the same thing".
