...
| `sandbox.host_fs` | string | `"legacy"` | — | Bubblewrap host-filesystem exposure (#1002 slice 4): `"legacy"` ro-binds the whole host `/` into every sandbox (deprecated — a startup warning is logged); `"allow_set"` mounts only the gateway-asserted set (workspace rw, toolchain roots ro, SDK tree, session-content + declared mounts, fresh `/proc`). Docker/microvm/wasm tiers are unaffected (they never bind the host root). Default flips after launch. |
