...
| `http_port` | u16 | `4100` | HTTP ingress bind (`0.0.0.0:http_port`) for `/api/*` routes (`event.ingest`, SSE, content API). Set `0` to disable HTTP while keeping localhost JSON-RPC on `port`. The web dashboard is served at `/` and `/index.html`; it is embedded in the binary at compile time — build with `--features dev-web` (e.g. `cargo run -p autonoetic --features dev-web --bin autonoetic run`) to serve `web/index.html` from disk instead, so page edits live-refresh without a rebuild or restart. |
...
| `stuck_task_warn_secs` | u64 \| null | `600` | WARN interval for a workflow task executor that keeps refreshing its claim heartbeat without completing its turn. A fresh heartbeat proves the gateway process is alive, not that the turn progresses — a turn hung inside a single tool call stays exempt from stuck-task recovery (`stuck_task_timeout_secs`) forever. At each multiple of this threshold the executor's heartbeat loop emits a WARN. `null` uses the default (600). Set to `0` to disable. |
| `stuck_task_hard_timeout_secs` | u64 \| null | `1800` | Hard cap on a workflow task executor's turn lifetime. Past this the heartbeat loop adjudicates the task itself — `Failed` with `stuck_executor` semantics, `stuck_failed` checkpoint, dequeue, and a `task.stuck` workflow event — even though the claim heartbeat is fresh, because a live-but-hung executor would otherwise block its workflow silently (every re-spawn path also blocks on the execution lock the hung turn still holds). `null` uses the default (1800). Set to `0` to disable (not recommended). |
...
| `trust_unsigned_bundles` | bool | `false` | Allow revision creation without a gateway signature when the identity key is unavailable (dev escape hatch). See `docs/reference/revision-signing.md`. |
...
```yaml
constitution:
  source_path: "runtime/constitution/versions/2026.09.05/constitution.md"
  lock_path: "runtime/constitution/versions/2026.09.05/gateway-constitution.lock.json"
  require_signature: true
  trusted_signers:
    autonoetic:constitution:v1: "lNxT1b/jWa6LqM2Thd7rW1IppvlH3rlEnAOPV81Igzk="
```
...
| `sandbox.dev_mode` | string | `"legacy"` | `AUTONOETIC_BWRAP_DEV_MODE` (gated) | `/dev` mount strategy: `"legacy"` (emit no override), `"minimal"` (`--dev /dev` — bwrap's own devtmpfs: null/zero/full/random/urandom/tty), `"host-bind"` (`--dev-bind /dev /dev`, least isolated). Under `host_fs: allow_set` (the default) `"legacy"` resolves to `"minimal"`: allow_set builds the root from `--tmpfs /` plus an explicit bind list with no `/dev`, so emitting no override would leave the sandbox without `/dev/null` and break the `2>/dev/null` in ordinary install scripts. `host_fs: legacy` already carries `/dev` in with its ro-bind of `/`, so `"legacy"` stays a no-op there. |
| `sandbox.allowed_mount_roots` | string list | `[]` | — | Operator **read-only** allowlist for agent-declared host mounts (#1002): a SKILL.md `runtime.mounts` entry is granted iff its canonicalized path is equal to or under one of these roots. Empty (default) denies every declared mount — the exec is raised for operator approval as a session-scoped **mount grant** (#1002 slice 5) when a store is available, or fails with a structured `mount_denied` refusal otherwise. `~` expands against the gateway user's home. Paths overlapping the gateway directory or any operator deny-list entry are never granted, by allowlist or by approval — mounts cannot shadow the sandbox secret mask. |
| `sandbox.allowed_mount_roots_rw` | string list | `[]` | — | Read-**write** ceiling: a `readonly: false` declaration is granted only when its canonicalized path is also at/under one of these roots (or approved as rw via a session mount grant, which carries the rw intent on the request). Read-only roots cannot be widened by a manifest — the request is denied with a refusal naming this key. |
| `sandbox.host_fs` | string | `"allow_set"` | — | Bubblewrap host-filesystem exposure (#1002 slice 4, DP-1 default flip): `"allow_set"` mounts only the gateway-asserted set (workspace rw, toolchain roots ro, SDK tree, session-content + declared mounts, fresh `/proc`); `"legacy"` ro-binds the whole host `/` into every sandbox (deprecated opt-out — a startup warning is logged). Docker/microvm/wasm tiers are unaffected (they never bind the host root). |
...
See `docs/reference/response-contract.md` for implementation details and `docs/guide/runbooks/iteration-repair-validation.md` for the repair runbook.
...
See `docs/guide/human-agent-collaboration.md` for the broader collaboration workflow.
...
See `docs/reference/protected-agents.md` for the full manual recovery procedure.

---

## Capability Delta Gate

Controls how capability broadening during `agent.revision.promote` is gated. High-risk promotion evidence (evaluator/auditor, federation jury) is separate — see `docs/AGENTS.md` and `docs/reference/protected-agents.md`.
...
See `docs/reference/schema-enforcement.md` for details.
...
See `docs/internals/code-analysis.md` for details.
...
| `loop_guard.idle_loop_floor_secs` | u64 | `60` | **Idle-loop floor.** A loop iteration whose wall-clock span between consecutive `check_loop` calls reached this many seconds is treated as sanctioned waiting — a blocking `workflow_wait`, a slow child join, a slow LLM round — and resets `current_loops` before the increment. A spin is *fast* no-progress iterations; real waiting throttles itself, so it must not accumulate into the P-7.18 degraded-mode sub-trip (the observed false positive: a planner polling a ~110 s-TTFB child in ~30 s rounds was degraded, and the degradation flooded to a fresh retry child). Tool-failure budgets are unaffected. Set to `0` to disable. |
...
tool demotion). See [docs/internals/prompt/budget.md](../internals/prompt/budget.md).
...
Controls the canonical session timeline (see [`docs/internals/session/room.md`](../internals/session/room.md)). The timeline uses an altitude model (`Detail < Normal < Attention < Error`) where each event's effective altitude is `max(base_altitude(event_type), role_floor(seat))`. Role floors only *raise* events, never suppress them.
...
stuck_task_warn_secs: 600
stuck_task_hard_timeout_secs: 1800
