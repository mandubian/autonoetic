...
- **Multi-field credentials**: a flow that collects several secret fields in one credential (e.g. a `user_prompt` step with multiple `secret_fields`) stores them two ways: each field under its own vault key (so `{{secrets.<field>}}` templates keep working), and a combined flat JSON object (`{"<field>": "<value>", …}`) under the credential id, which the credential record points at. At spawn time the combined object is injected under the credential's env-var name, **and** each field is additionally injected under `<SERVICE>_<FIELD>` (both parts uppercase-sanitized) — scripts can consume either the combined JSON or individual variables. The credential's own env-var name always wins: an `inject_as` that collides with one of the credential's derived field names (`inject_as: PHOTOS_ACCOUNT_NAME` on a credential holding an `account_name` field) keeps the combined object under that name and drops the field var (logged loudly).
...
`credential_check` reports `secret_present` per credential (vault presence, without exposing the value) so onboarding flows can distinguish "no credential record" from "record exists but vault secret missing". It also reports `created_at` / `updated_at` (RFC 3339, gateway clock — `updated_at` bumps on every re-set), so an agent can answer "which credential is the latest" from metadata alone. `credential_check` and `credential_setup` also return an `injection` block per credential — `env_var` (the effective injection name), `value_shape` (`scalar` or `json_object`), and for multi-field credentials the `fields` list plus the derived `field_env_vars` — so an agent can verify the exact contract a spawned script will see before running it.

### Updating `inject_as` on an existing credential

Re-calling `credential_setup` for an existing `(service, label)` pair dedups: it returns `existing: true` and the stored credential instead of re-running the ceremony. If the retry passes a different `inject_as`, the gateway **persists the update** on the existing record (response carries `inject_as_updated: true`) and reports the new `injection` block — only the metadata changes, the secret value is untouched, and env-name safety (`PATH`, `LD_*`, …) is enforced at injection time. This is the supported repair path when a credential was onboarded with a missing or wrong env-var name: retry with the correct `inject_as` instead of re-collecting the secret. The reported flag tracks what actually persisted: if the record disappeared between the dedup lookup and the write, the response carries `inject_as_updated: false` and a note to re-onboard, never a persistence claim the store did not honour.
...
If neither env var is set, `autonoetic agent bootstrap` (and gateway bootstrap of agents / single-agent install paths) creates `{runtime_dir}/vault.key` when that file is missing, using the same logic as `vault::ensure_default_key`.
