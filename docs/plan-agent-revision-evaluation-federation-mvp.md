...
- [x] Ensure approval resume, checkpoint resume, and retry paths always reload from binding state for execution. Gateway policy and validation decisions (emergency stop, spawn permissions, response-contract fallback) intentionally consult the current alias state.
...
- [x] A running session is fully determined by its stored binding for execution (agent files, runtime closure, layer mounts). Gateway policy and validation decisions (emergency stop, spawn permissions, response-contract fallback) intentionally consult the current alias state.
