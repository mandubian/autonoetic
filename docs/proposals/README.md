...
| [`constitution-bind-direction-model.md`](constitution-bind-direction-model.md) | Open | Bind-direction as data, not prefix convention — so the constitution is re-implementable. `binds()` currently derives the bound party from the ID prefix and returns `Agent` for `P-8.1`/`P-15.1`; `Binds` cannot express the served party; §0 ("exactly one party") contradicts §12 ("the community… collectively") |
