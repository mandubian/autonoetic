...
| [`constitution-bind-direction-model.md`](constitution-bind-direction-model.md) | Partial | **rev 3.** Parts 1–2 **complete** (#1293, #1298, #1302, #1304, #1306, #1307, #1309, #1310): `binds`/`owed_to`/`requires` are declared data for **all 221 clauses** (`constitution_relations`, rendered to [`../constitution/law-table.md`](../constitution/law-table.md)); `binds()` reads the field; section-grouping inference is **0**; `requires` is mandatory on the record, so its coverage is a type property. Open: §2.4.1's `achieved` still unmoved (needs §2.7's per-implementation register); part 3 (the amendment adopting the columns) not drafted — needs the signing key. Tracking #1284 |
