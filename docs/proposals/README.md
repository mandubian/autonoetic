...
| [`constitution-bind-direction-model.md`](constitution-bind-direction-model.md) | Partial | **rev 3.** Parts 1–2 shipped (#1293, #1298, #1302, #1304, #1306, #1307): `binds`/`owed_to` are declared data for **124 of 221** clauses, section-grouping inference is **0**, §2.4.1's `requires` is implemented for the first 40, and §2.4.2's reader surface shipped as [`../constitution/law-table.md`](../constitution/law-table.md). Open: `requires` for the 84 numbered `P-*` already classified; `achieved` still unmoved (needs §2.7's per-implementation register); 97 `P-*` unclassified; part 3 (the amendment) not drafted. Tracking #1284 |
...
| [`readme-presentation.md`](readme-presentation.md) | Partial | Front-page rewrite: the README explains the constitution with the prefix model #1284 retired. Truth pass, the declared bind-direction model in one screen, two diagrams replacing the mermaid plate, and a clause-ID guard for markdown |
