...
| [`constitution-bind-direction-model.md`](constitution-bind-direction-model.md) | Partial | **rev 3.** Parts 1–2 shipped (#1293, #1298, #1302): `binds`/`owed_to`/`verified_by` are declared data, `binds()` reads the field, and 73 clauses are classified law-side in `constitution_relations`. Open: §2.4.1 reshapes `verified_by` into `requires` (constitutional) + `achieved` (register) before the amendment adopts columns; §2.4.2 the classification has no reader-facing surface (54 of 73 clauses render nowhere); 148 numbered `P-*` unclassified. Tracking #1284 |
