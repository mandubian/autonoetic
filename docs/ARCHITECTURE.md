...
- [Session lifecycle, causal chain, and the digest](#session-lifecycle-causal-chain-and-the-digest)
...
- **`ViewerClass` (Agent / Decider / Operator / Admin)** controls *what an observability or approval reader sees* based on who they are. Agents reading `execution_search` get trace metadata only — no stdout, no commands, no arguments, no result. Agents reading `approval_summary` for `WriteFile` see the path but not content; for `CredentialRequest` they see only `credential_id`/`url`/`method` (headers / body / payload blanked). `SandboxExec.command` is blanked for the Agent class as well (issue #158, fixed by PR #160). An agent holding an active decider appointment over the run reads at the `Decider` class for gates in that scope (#1194): operator-shaped, so it sees the command it must judge, but with embedded secrets masked in place — its reads are causal-chained, so what it sees is what the record keeps. Operators see structural fields with secret-named JSON keys redacted; non-JSON strings get precise in-place masking via `redact_embedded_secrets`. Admins see the raw record. Class is selected at the call site.
...
Content-addressable storage (SHA-256 handles, natural deduplication) that works
locally and for remote agents over HTTP. Content is named within a session, with
visibility controlling whether siblings and the root session can read it.

The on-disk layout, the visibility model, the manifest structure, and the API are
in [`internals/storage/content-store.md`](internals/storage/content-store.md);
the reachability rules that make `Private` a guarantee are in
[`internals/storage/content-visibility.md`](internals/storage/content-visibility.md).

## Session lifecycle, causal chain, and the digest

Every session produces a hash-chained audit trail; every yield point produces a
runnable checkpoint; causal events are mirrored into SQLite so agents can query
their own history; and the live digest renders it as narrative.

The mechanics — entry structure, checkpoint kinds, turn suspension, auto-resume,
forking, the event tables, contract health, the digest format, and the session
read cache — are in
[`internals/session/lifecycle.md`](internals/session/lifecycle.md).
...
All transactional state lives in one SQLite database, `runtime/gateway.db`, with
ordered schema migrations. One database rather than several is what makes a
session's approvals, workflow events, causal mirror, and revision bindings
mutually consistent without cross-store coordination.

Every table — grouped by domain, with owner module, relations, and usage status,
audited against `migrate.rs` — is in
[`reference/store-schema.md`](reference/store-schema.md). Retention policy
(pruning `execution_traces`, `causal_events`, and `post_promotion_reviews`) is
configured under `retention` in the gateway config; see
