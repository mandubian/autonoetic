...
| `max_probes_per_host` | u32 | `3` | Per-host `sandbox_exec` probe budget (#853). Maximum "wasted" probes against a single host per session before the next one is refused with `host_budget_exhausted`. A probe is wasted when it fails, or when it succeeds but returns content already seen from that host (the SPA / rate-limited-endpoint retry loop the rotating-poll guard misses because each script differs); a novel success resets the count. Scoped per exact `session_id`, so a re-spawn with different instructions gets a fresh budget. The trip surfaces an Attention-altitude operator signal at the root. Set to `0` to disable. |
