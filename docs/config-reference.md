...
| `prompt_budget.compress_tool_schemas_after_turn_0` | bool | `false` | Replace tool JSON schemas with `{"type": "object"}` after the first turn to save tokens. **Disabled by default** — stripping schemas causes tool-call divergence (wrong params, missing required fields). The context governor still compresses schemas as a last resort when the budget is exceeded. |
...
| `prompt_budget.max_tool_result_chars` | usize | `2000` | Maximum characters to allow in a tool-result message content before it is truncated to head + tail for the LLM request. Large stdout/stderr blobs are not replayed in full every turn, but the full result is still stored. Set to `0` to disable truncation. |
...
tool-schema compression → tool demotion). See [docs/prompt-budget.md](prompt-budget.md).
