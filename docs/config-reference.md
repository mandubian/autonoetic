...
| `stuck_task_timeout_secs` | u64 \| null | `600` | Max seconds a `Running` workflow task without progress before force-completion. The sweeper checks whether the child session has actually completed before resolving. `null` uses the default (600). Set to `0` to disable. |
