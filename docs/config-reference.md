...
| `prompt_budget.strip_reasoning_from_request` | bool | `true` | Strip `reasoning_content` / `reasoning_details` from assistant messages before sending them to the LLM. The model does not need to re-read its own chain-of-thought, and checkpoints/exports/timeline keep the full reasoning for audit. |
