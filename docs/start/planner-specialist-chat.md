...
See `docs/reference/config.md` for the full configuration reference.
...
# See docs/internals/prompt/budget.md for details.
prompt_budget:
...
- **Session transcripts** are automatically persisted at hibernation and session close and indexed with SQLite FTS5 for full-text search. Agents can search past sessions with `session_search` and summarize them with `session_peek` (see `docs/guide/session-search.md`).
...
- `docs/reference/agent-adapter-contract.md`
...
See `docs/reference/credentials.md` for details.
...
```
For architecture details, see `docs/internals/approval-delivery.md`.
...
- `docs/reference/config.md` — full `config.yaml` field reference
...
- `docs/reference/credentials.md` — secure credential management
- `docs/guide/session-search.md` — FTS session search
- `docs/internals/workflow-orchestration.md` — workflow orchestration
